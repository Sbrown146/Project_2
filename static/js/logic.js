  ...
  //change these to a config.js before uploading
  ...
  let nba_url = "https://api.sportsdata.io/v3/nba/scores/json/Stadiums?key=347c7cf39a8b4f58906ce63f2ac52906";
  var baseMaps = {};
  var overlayMaps_NHL= {};
  var overlayMaps_MLB= {};
  var mlb = [];
  ...
  L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1Ijoic2Jyb3duMTQ2IiwiYSI6ImNrMWUycThmajBhZzIzYm11bjRqMDRvamQifQ.M22-sX0fWx_QGYGZQpRDGw", {
    ...
    // L.control.layers(overlayMaps).addTo(Init_map);
  ...
  d3.json(mlb_uri, function(data){
    ...
    for (let j of mlb_index_lookup) {
        ...
        for (let i = 0; i < mlb_teams.length; i++) {
          ...
          }).bindPopup("<h1>" + mlb_teams[i].name + "</h1> <h2> MLB: " + mlb_teams[i].stadium_name + "</h2> <hr> <h3>Altitude: " + mlb_teams[i].altitude + "</h3>").addTo(Init_map));
        }
      }
  ...
  d3.json(nhl_uri, function(data){
    ...
    for (let j = 0; j < 30; j++) {
        ...
        for (let i = 0; i < nhl_teams.length; i++) {
          ...
          }).bindPopup("<h1>" + nhl_teams[i].name + "</h1> <h2> NHL: " + nhl_teams[i].stadium_name + "</h2> <hr> <h3>Altitude: " + nhl_teams[i].altitude + "</h3>").addTo(Init_map));
        }
      }

      //Have a league set as default and change when league is changed.

      // nhl = L.layerGroup(nhl_markers);
      // overlayMaps_NHL = { "NHL": nhl};
      // L.control.layers(overlayMaps_NHL).addTo(Init_map);
  ...
  d3.json(nfl_uri, function(data){
    ...
    for (let j = 0; j < 31; j++) {
        ...
        for (let i = 0; i < nfl_teams.length; i++) {
          ...
          }).bindPopup("<h1>" + nfl_teams[i].name + "</h1> <h2> NFL: " + nfl_teams[i].stadium_name + "</h2> <hr> <h3>Altitude: " + nfl_teams[i].altitude + "</h3>").addTo(Init_map));
        }
      }
  ...
  d3.json(nba_uri, function(data){
    ...
    for (let j = 0; j < 30; j++) {
        ...
        for (let i = 0; i < nba_teams.length; i++) {
          ...
          }).bindPopup("<h1>" + nba_teams[i].name + "</h1> <h2> NBA: " + nba_teams[i].stadium_name + "</h2> <hr> <h3>Altitude: " + nba_teams[i].altitude + "</h3>").addTo(Init_map));
        }
      }
